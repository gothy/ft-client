...
(function() {
  var ApiError, FTClient, NetworkError, base_url, fileCopy, fileDelete, fileMove, fileRename, fileUpload, folderContent, folderCopy, folderCreate, folderDelete, folderInfo, folderMove, folderRename, token, trashcanContent, trashcanEmpty, trashcanRestore, userInfo, userLogin, _do_file_upload, _file_method_helper, _fldr_ops, _folder_method_helper, _trashcan_method_helper,
  ...
  _trashcan_method_helper = function(method, params, cb) {
    var _this = this;
    params.token = params.token || token;
    return $.ajax({
      ...
      if (data.status === 200) {
        if (method === 'content') {
          result = data != null ? (_ref = data.response) != null ? _ref.files : void 0 : void 0;
        } else {
          result = data != null ? (_ref1 = data.response) != null ? _ref1.result : void 0 : void 0;
        }
        if (cb && typeof cb === 'function') {
  ...
  trashcanContent = function(params, cb) {
    return _trashcan_method_helper('content', params, cb);
  };

  trashcanEmpty = function(params, cb) {
    return _trashcan_method_helper('empty', params, cb);
  };

  trashcanRestore = function(params, cb) {
    return _trashcan_method_helper('restore', params, cb);
  };
